...
def sanitize_mineru_cells(cells: List[TextCell], ignore_rotated: bool = False):
    ...
    if ignore_rotated:
        removed_cells = [cell for cell in cells if abs(cell.info["line_angle"]) >= 1]
        cells = [cell for cell in cells if abs(cell.info["line_angle"]) < 1]
        if len(removed_cells) > 0:
            _log.info(f"Removed {len(removed_cells)} rotated cells")
